module.exports = {
	...
	run: async (client, message, args) => {
		...
		const age = args.join(' ');
    if (!age) return message.channel.send(`:x: | **No age provided \n Usage : +fetch-alts 30d**`)
		const ms = client.parseMs(age);
		if (!ms) return message.channel.send(':x: | **Invalid age provided | set age as a days only!.**');
		const alts = members
    ...
    if (!alts.length) return message.channel.send(":x: | **No alts found**");
		...
		let e = new Discord.MessageEmbed()
			...
			.setFooter(`Page ${page + 1} of ${pages.length} (${alts.length} entries)` + " | Blob World")
			.setColor('RANDOM');
		...
		while (doing) {
			...
			try {
				...
				return message.channel.send(':timer: | **Command timed out.**');
			}
			...
			if (r.emoji.name == symbols[0]) {
				if (!pages[page + 1])
					...
					let newEmbed = new Discord.MessageEmbed()
						.setDescription(pages[page].join('\n'))
						.setFooter(
							`Page ${page + 1} of ${pages.length} (${alts.length} entries)` + " | Server Director 2021"
						)
						.setColor('RANDOM');
					msg.edit(newEmbed);
				}
				...
				if (!pages[page - 1])
					...
					let newEmbed = new Discord.MessageEmbed()
						.setDescription(pages[page].join('\n'))
						.setFooter(
							`Page ${page + 1} of ${pages.length} (${alts.length} entries)` + " | Server director"
						)
						.setColor('RANDOM');
					msg.edit(newEmbed);
				}
